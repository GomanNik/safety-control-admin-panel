...
class VisionRuntimeService:
        ...
        with self._control_lock:
            ...
            try:
                self._settings.validate_runtime_launch_or_raise(
                    ...
                    require_real_headwear=self._settings.runtime_require_real_headwear,
                )
            except ValueError as error:
                return False, str(error)

            readiness_errors = self._collect_startup_readiness_errors(source_url=effective_source_url)
            if readiness_errors:
                return False, "Processed video export rejected: " + "; ".join(readiness_errors)

            self._set_export_running(True, "Processed video export started.")
            ...
            thread.start()
            return True, "Processed video export started. Poll GET /runtime/status for progress."
        ...
        *,
        ...
        for bundle in bundles:
            ...
            if not run_headwear:
                ...
                headwear = self._headwear_detector.assess_observation(frame=frame, observation=bundle.observation)
            ...
            if run_headwear and self._headwear_incidents_are_enabled():
                incident_result = self._incident_engine.process_headwear_assessment(
                    observation=bundle.observation,
                    headwear_assessment=headwear,
                    frame_path=None,
                )
            ...
            self._metrics.record_observation(
                ...
                head_bbox=RuntimeMetricsRecorder.bbox_to_text(bundle.observation.head_bbox),
    ...
    def _capture_violation_evidence_after_incident_decision(
        ...
        *,
        ...
        record = self._frame_store.save_incident_evidence(
            ...
            head_bbox=observation.head_bbox,
            quality_score=observation.quality_score,
    ...
    def _collect_startup_readiness_errors(self, *, source_url: str) -> list[str]:
        errors: list[str] = []
